...
def enter(user, reply):
	reply(
		'Ты открываешь дверь и видишь целый караван циган!\n'
		'К тебе подходит серобородый циган в костюме твоего дяди и начинает разговор:\n\n'
...
def buy(user, reply, name):
	...
	if name in names:
		...
		if user.paid(costs[ind]):
			...
			reply('Циган ехидно улыбается и не отдает тебе вещь.\n«Нет денег — нет товара».')
...
def action(user, reply, text):
	...
	if question == 'first':
		if text == FIRST_ACTIONS[0]:
			...
			reply(
				'Ты избил всех циган (Благо они маленькие и ты просто пинал их по лицу х*ем) и '
