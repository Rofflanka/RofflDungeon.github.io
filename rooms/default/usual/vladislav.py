...
def enter(user, reply):
	msg = (
		'Вы заходите в комнату и, бросив мимолётный взгляд на человека, парящего вейп '
...
def action(user, reply, text):
	...
	if question == 'first':
		if text == TOOK_TOOK:
			...
			reply('Начав движение, из-за спины Вы слышите голос с небольшим пидорским акцентом: — Ты долбаёб?')
			user.set_room_temp('question', 'spine')
	elif question == 'spine':
		reply('Вы испытываете сильный стыд и так краснеете, что у Тебя сужается очко.')
